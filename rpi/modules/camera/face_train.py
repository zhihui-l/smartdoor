...
Face_Images = os.path.join(os.getcwd(), "img") #Tell the program where we have saved the face images 
print (Face_Images)
...
for root, dirs, files in os.walk(Face_Images): #go to the face image directory 
    for file in files: #check every directory in it 
			...
			#print(Numpy_Image)
            faces = face_cascade.detectMultiScale(Final_Image) #Detect The face in all sample image 
            print (Face_ID,faces)
