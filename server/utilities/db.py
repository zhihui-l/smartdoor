...
def getUser(uid = None):
    global cursor, db
    if uid:
        cursor.execute("SELECT name FROM user WHERE uid = %d" % int(uid))
    else:
...
def getLog():
    global cursor, db
    cursor.execute("SELECT l.lid, l.type, l.uid, u.name, l.createtime, l.img, l.data FROM log l LEFT JOIN user u ON l.uid = u.uid ")
    return cursor.fetchall()
